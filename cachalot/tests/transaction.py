...
class AtomicTestCase(TestUtilsMixin, TransactionTestCase):
    ...
    def test_successful_write_atomic(self):
        ...
        with self.assertNumQueries(4 if self.is_dj_21_below_and_is_sqlite() else 3):
            with transaction.atomic():
    ...
    def test_successful_nested_read_atomic(self):
        with self.assertNumQueries(7 if self.is_dj_21_below_and_is_sqlite() else 6):
            with transaction.atomic():
    ...
    def test_unsuccessful_nested_read_atomic(self):
        with self.assertNumQueries(6 if self.is_dj_21_below_and_is_sqlite() else 5):
            with transaction.atomic():
                try:
                    with transaction.atomic():
                        with self.assertNumQueries(1):
    ...
    def test_successful_nested_write_atomic(self):
        with self.assertNumQueries(13 if self.is_dj_21_below_and_is_sqlite() else 12):
            with transaction.atomic():
                t1 = Test.objects.create(name='test1')
                with transaction.atomic():
                    t2 = Test.objects.create(name='test2')
    ...
    def test_unsuccessful_nested_write_atomic(self):
        with self.assertNumQueries(16 if self.is_dj_21_below_and_is_sqlite() else 15):
            with transaction.atomic():
                t1 = Test.objects.create(name='test1')
                try:
                    with transaction.atomic():
